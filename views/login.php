...
<div class="sml-login" id="spiral-member-login<?php $template->the_template_num(); ?>">
	...
	<form onsubmit="return befor_submit()" name="loginform" id="loginform<?php $template->the_template_num(); ?>" action="<?php $template->the_auth_form_url(); ?>" method="post">
		...
		<p class="submit">
			<input type="hidden" name="identification_key" value="<?php echo get_option('spiral_member_login')["wpmls_member_identification_key"]; ?>" />
			<input type="hidden" name="area_title" value="<?php echo get_option('spiral_member_login')["wpmls_area_title"]; ?>" />
