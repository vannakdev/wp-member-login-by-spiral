...
<div class="sml-login" id="spiral-v2-member-login<?php $template->the_template_num(); ?>">
	...
	<form name="loginform" id="loginform<?php $template->the_template_num(); ?>" action="<?php $template->the_auth_form_url(); ?>" method="POST">
		...
		<p class="submit">
			...
			<input type="hidden" name="site_id" value="<?php echo get_option('spiral_v2_member_login')["wpmls_site_id"]; ?>" />
			<input type="hidden" name="authentication_id" value="<?php echo get_option('spiral_v2_member_login')["wpmls_authentication_id"]; ?>" />
