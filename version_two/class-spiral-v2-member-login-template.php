 ...
 */
	class WPMLS_Spiral_Member_Login_Template extends WPMLS_Spiral_Member_Login_Base
	{
		...
		public function display($action = '')
		{
			...
			if ($spiral_member_login->is_settings_imcomplete()) {
				if (current_user_can('manage_options')) {
					...
					echo '<div class="error"><strong>Error</strong>: WP Member Login by SPIRALV2 plugin error. </div>';
			...
			if ($this->get_option('show_title')) {
				$login_text = (get_locale() == "en_US") ? 'Log In' : ((get_locale() == "ja") ? 'ログイン' : 'ログイン');
				echo '<p class="sml-user-links-title">'.$login_text.'</p>';
			}
			if (has_action('sml_display_' . json_encode($action))) {
				...
				if ($spiral_member_login->is_logged_in()) {
					...
					if($action["is_widget"]){
						$templates[] = 'user_info_v2_widget.php';
					}else{
						if(isset($action['hide_logout_link']) && $action['hide_logout_link'] == true){
					...
					if(isset($action['is_widget']) != NULL)
						$templates[] = 'login_v2_widget.php';
					$templates[] = 'login_v2.php';
				}
				...
				if ($page_id = Spiral_Member_Login::get_page_id($action)) {
					...
					switch ($action) {
						...
						default:
							...
							$title .= '<p class="sml-user-links-title">' . $login_text . "</p>";
					}
				}
			}
		...
		{
			$target = get_option('is_blank_page') == "1" ? 'target="_blank"' : "";
			if ($action_links = $this->get_action_links($args)) {
				echo '<div>' . "\n";
				echo '<ul>' . "\n";
				foreach ((array) $action_links as $link) {
					echo '<li class="sml-action-links-'. $link['class'] .'"><a '.$target.' href="' . esc_url($link['url']) . '" rel="nofollow">' . $this->translator->sml_translate($link['title']) . '</a></li>' . "\n";
				}
				echo '</ul>' . "\n";
				echo '</div>';
			}
		...
		/**
		 * Outputs the action links for widget
		 *
		 * @since 1.0.0
		 * @access public
		 *
		 * @param array $args Optionally specify which actions to include/exclude. By default, all are included.
		 */
		public function the_action_links_widget($args = '')
		{
			if ($action_links = $this->get_action_links($args)) {
				echo '<div>' . "\n";
				echo '<ul>' . "\n";
				foreach ((array) $action_links as $link) {
					echo '<li class="sml-action-links-'. $link['class'] .'"><a target="_blank href="' . esc_url($link['url']) . '" rel="nofollow">' . $this->translator->sml_translate($link['title']) . '</a></li>' . "\n";
				}
				echo '</ul>' . "\n";
				echo '</div>';
			}
		 ...
		 * @param array $args Optionally specify which actions to include/exclude. By default, all are included.
		 */
		public function the_action_links_widget_target_blank($args = '')
		{
			if ($action_links = $this->get_action_links($args)) {
				echo '<div>' . "\n";
				echo '<ul>' . "\n";
				foreach ((array) $action_links as $link) {
					echo '<li class="sml-action-links-'. $link['class'] .'"><a href="' . esc_url($link['url']) . '" rel="nofollow">' . $this->translator->sml_translate($link['title']) . '</a></li>' . "\n";
				}
				echo '</ul>' . "\n";
				echo '</div>';
			}
		...
		/**
		 * Outputs logged-in user links for widget
		 *
		 * @since 1.0.0
		 * @access public
		 */
		public function the_user_links_widget()
		{
			$spiral_member_login = Spiral_Member_Login::get_instance();
			echo '<ul class="sml-user-links">';
		 ...
		 * @access public
		 */
		public function the_user_links_widget_target_blank()
		{
			$spiral_member_login = Spiral_Member_Login::get_instance();
			echo '<ul class="sml-user-links">';
